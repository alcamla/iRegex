...
func matchesAsTextChecking(pattern:String, inString string:String)->[NSTextCheckingResult]?{
    
    do{
        let regex  = try NSRegularExpression(pattern: pattern, options: NSRegularExpressionOptions(rawValue: 0))
        let range = NSMakeRange(0, string.characters.count)
        return regex.matchesInString(string, options: NSMatchingOptions(rawValue:0), range: range)
...
/*:
 ### Testing regex for Core Text Tutorial
 */

let markup = "These are <font color=\"red\">red<font color=\"black\"> and <font color=\"blue\">blue <font color=\"black\">words."
listMatches("(.*?)(<[^>]+>|\\Z)", inString: markup)

let chunks = matchesAsTextChecking("(.*?)(<[^>]+>|\\Z)", inString: markup)
let chunk = chunks![0]
    let parts = (markup as NSString).substringWithRange(chunk.range).componentsSeparatedByString("<")
    print(parts)
